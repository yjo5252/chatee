...
![1](https://user-images.githubusercontent.com/41438361/88438146-fa41c880-ce42-11ea-9237-72b6f431ef59.JPG)

`OnMembersAddedAsync` 함수에서 사용자가 처음 봇과 대화를 시작할 때 환영 인사를 합니다.
...
`MainDialog.is_running_dialog`가 0이면 현재 실행중인 Dialog가 없다는 말이고(현재 실행중인 기능이 없다. 기능 실행 명령을 대기중이다.) 이때는 사용자의 입력을 받아 사용자의 입력 메세지에 특정 기능을 나타내는 키워드가 있을 경우 해당 기능을 실행시킬 수 있도록 모드를 설정합니다.

![5](https://user-images.githubusercontent.com/41438361/88438660-34f83080-ce44-11ea-9eda-2a12b6e9906a.JPG)

특히, 사용자 입력에 "목적" 이나 "안녕" 이라는 단어가 포함되면 QnA Maker가 실행될 수 있도록 해놨습니다. 이 QnA Maker가 정상적으로 작동하기 위해서는 QnA Maker를 불러오는 코드가 존재하며 `appsettings.json` 파일에 연결 설정이 잘 되어 있어야 합니다. 

![6](https://user-images.githubusercontent.com/41438361/88438847-90c2b980-ce44-11ea-9d9c-cd2f8a981ea5.JPG)

QnA Maker를 불러오는 코드는 위와 같습니다. 이 코드를 QnA Maker를 실행시키는 부분 전에 선언만 해주면 됩니다.

`appsetting.json`을 설정하는 법은 뒷 부분에 설명되어 있습니다.
...
##### `Dialogs/ShowFunctionsDialog.cs`
...
##### `Dialogs/RecommendExerciseDialog.cs`
...
##### `Dialogs/SeeMyCharacterDialog.cs`
